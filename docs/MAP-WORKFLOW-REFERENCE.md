...
**CRITICAL: Use Task tool to invoke all subagents. NEVER use Bash or mapify commands.**
...
Task(
  ...
  prompt="Break down this goal into JSON subtasks (≤8), each with acceptance criteria. Context: <files/constraints>"
)
```

- **DECOMPOSE**: Task(subagent_type="task-decomposer", prompt="Break down this goal into JSON subtasks (≤8), each with acceptance criteria. Context: <files/constraints>")
- **IMPLEMENT**: Task(subagent_type="actor", prompt="Implement this subtask. Provide Approach, Code Changes (full content), Trade-offs, Testing, and Used Bullets list. Context: <files/constraints> Playbook: <relevant_bullets>")
- **VALIDATE**: Task(subagent_type="monitor", prompt="Validate the proposal. Output strict JSON with issues and verdict.")
- **PREDICT**: Task(subagent_type="predictor", prompt="Analyze impact. Output strict JSON with affected files, breaking changes, and required updates.")
- **EVALUATE**: Task(subagent_type="evaluator", prompt="Score solution quality. Output strict JSON with scores and recommendation.")

### ACE Learning Agents

- **REFLECT**: Task(subagent_type="reflector", prompt="Extract structured lessons from this attempt. Provide: actor_code, monitor_results, predictor_analysis (if available), evaluator_scores (if available), execution_outcome. Output strict JSON with: reasoning, error_identification, root_cause_analysis, correct_approach, key_insight, bullet_updates, suggested_new_bullets.")

- **CURATE**: Task(subagent_type="curator", prompt="Integrate Reflector insights into the playbook. Provide: current_playbook (from .claude/playbook.json), reflector_insights (JSON from Reflector). Output strict JSON with: reasoning, operations (ADD/UPDATE/DEPRECATE), deduplication_check, sync_to_cipher.")
