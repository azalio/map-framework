...
- Decisive Validation: `python "/Users/azalio/.config/opencode/skills/improvement-plan-loop/scripts/idea_index.py" .`, focused workflow-fit regression tests, template sync tests, `make lint`, `pytest -m "not slow"`, and a repo-built generated-project smoke verified the plan/done state and shipped off-ramp artifacts.
...
- Reusable Learnings:
  ...
  - review-check: `Before implementing an active idea, grep runtime, shipped templates, docs, and tests for the core artifact/route names; close stale plan entries instead of rebuilding shipped behavior.`
