...
At workflow completion, the `scrub-internal-ids.py` Stop hook removes MAP-internal workflow IDs (`ST-`/`AC-`/`VC-`/`INV-`/`HC-`) that leaked into the code a run changed — in comments, string literals, and `vc<n>` test names — and commits the result as `chore(map): strip internal workflow IDs`. It is hard-scoped to the run's git diff (IDs you wrote yourself on untouched lines are never modified), runs exactly once per completed run, and can be turned off with `scrub_internal_ids: false` in `.map/config.yaml`. (Claude provider only — the Codex hook model has no `Stop` event; the engine still ships to `.map/scripts/`.)
