...
Once a plan reaches `ready` (task_plan + blueprint both present, the Step 7 boundary), you can start `/map-efficient` or `/map-task` right away — the plan needs no separate approval. A pending `plan_approval` hold left by an older mapify version is closed automatically by the `/map-efficient` and `/map-task` preflights.
...
Two role reviewers run in that same fan-out by default — no flag, no opt-in. `user_experience` asks one question: did the change make the ALREADY SHIPPED functionality harder or less convenient (extra mandatory steps on the old path, confusable flags such as `--release` vs `--from-release`, an explicitly-supplied value silently overridden instead of rejected), judged both as a human in a terminal and as a CI script. `maintainer` asks whether the next person can extend this without re-reading every line: branch-scoped plan/tracker IDs left in comments, implementation vocabulary in help/error text, errors the user cannot diagnose from one run, copy-paste, a decision computed in two places, version checks named by release number instead of capability, embedded shell/SQL/YAML in string literals, a setting carried through layers that only pass it on, a function name that promises more than its body does, a shared mechanism named after its first application, and files placed against the local convention — in four separate passes (correctness, setting propagation, names and boundaries, file structure). Both roles treat existing signatures, names, fields and responsibility splits as under review, review an attached plan as well as the code, and never modify the working tree. Every role finding must arrive with all five contract parts — `problem` (role, priority, every `file:line`), `current_code` verbatim, `proposed_code` applicable as a patch (imports, every call site, source→destination for moves), a MEASURABLE `why_better`, and its `cost` — plus a `verified_by` tier (`read`, `test_run`, `needs_environment`), or it is dropped as `contract_incomplete`, reported and never counted toward the verdict. Proposed patches are checked by reading only; the presentation states once that they were not built, linted or tested. Where it is dropped depends on the path: on the normal fan-out the ledger tombstones it and names it in `not_verified`, and when its severity is above `minor` it also sets `escalation_required`, so a CRITICAL or IMPORTANT claim lost to a missing `cost` costs the run its PROCEED instead of vanishing; under `--adversarial` the aggregator removes it before the ledger and lists it under `contract_incomplete` in the report. Under `--adversarial` both roles join the reviewer set (five reviewers; `--quick` drops only the Edge Case Hunter). To build just their prompts: `python3 .map/scripts/map_step_runner.py build_role_review_prompts [--roles user_experience,maintainer]`.
