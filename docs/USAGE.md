...
The order of sections inside each fan-out prompt is controlled by `prompt_layering` in `.map/config.yaml`. The default `docs_first` puts the variable `<documents>` (bundle, preferences, diff) before the stable `<task>`/`<instructions>`/`<expected_output>` contract — best for model attention. Setting `prompt_layering: stable_first` reorders the stable contract ahead of the documents so it becomes a byte-identical prefix across repeated same-role dispatches, which is the precondition for an automatic prefix-cache hit. This is opt-in and unproven: the attention-vs-cache tradeoff must be measured before changing the default — see "Prompt Layering & Prefix Caching" in `docs/ARCHITECTURE.md` for the measurement recipe. An absent or invalid value falls back to `docs_first`.
