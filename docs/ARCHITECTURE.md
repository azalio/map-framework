...
- **Review Verdict Ledger** (`/map-review`, issue #406): `write_review_verdict_ledger` in `map_step_runner.py` normalizes Monitor/Predictor/Evaluator outputs through a closed decision table (`review_verdict_table.v1`) and writes `.map/<branch>/review-verdict-ledger.json` (machine-readable audit trail) and `.map/<branch>/review-verdict-ledger.md` (human summary). Decision logic: CRITICAL findings → `BLOCK` regardless of category; security/correctness important findings → `BLOCK`; other important or `needs_investigation` findings → `REVISE`; all minor or no findings → `PROCEED`. Pre-existing findings (`was_present_before_pr=true`) are tombstoned and excluded from `computed_verdict`. MEDIUM/HIGH findings without `reach_evidence` are downgraded to `status="downgraded"` (non-active, non-blocking). Adversarial mode accepts `--adversarial-json`; compare-orderings mode records both ordering runs. The previous verdict is carried forward into the journal entry. A `computed_verdict` that differs from the skill's `FINAL_VERDICT` is a re-verify signal. CLI: `map_step_runner.py write_review_verdict_ledger --monitor-json … --predictor-json … --evaluator-json … --review-mode <normal|adversarial|cross_ai|compare_orderings>`. The `review_verdict_ledger` manifest stage is updated after write. 30 new tests; 4317 total tests pass.
...
- Review verdict ledger (#406): `write_review_verdict_ledger` normalizes
  ...
  Pre-existing findings tombstoned; MEDIUM/HIGH without reach_evidence
  downgraded to non-active. Adversarial and compare_orderings modes supported.
  `review_verdict_ledger` manifest stage. 30 new tests; 4317 total tests pass.
