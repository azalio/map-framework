...
- Internal-ID scrub: at run completion the `scrub-internal-ids.py` Stop hook removes MAP-internal workflow IDs (`ST-`/`AC-`/`VC-`/`INV-`/`HC-`) that leaked into run-changed code (comments, string literals, `vc<n>` test names) and commits the cleanup; the deterministic engine in `.map/scripts/scrub_internal_ids.py` is hard-scoped to the run's git diff and runs once per completed run (Claude provider only — Codex has no `Stop` event)
