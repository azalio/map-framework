...
- **Durable Approval-Hold Artifacts** (issue #344): parks blocked risky MAP actions with a stable id so a later session can inspect and decide without replaying chat history. When a safety guardrail, autonomy gate, or workflow policy blocks an action (e.g. force-push to main, template overwrite, destructive data operation), the runner calls `create_approval_hold(kind, reason, summary, source, continuation)` to write two branch-scoped artifacts: `.map/<branch>/approval_holds.json` (structured store keyed by `hold-NNN` ids with state, fingerprint, timestamps) and `.map/<branch>/approval_hold_<id>.md` (human-readable report with the decision instructions). Hold kinds: `safety_guardrail`, `autonomy_git_block`, `template_overwrite`, `workflow_approval`, `other`. State transitions: `pending → approved | denied | expired | cancelled` via `decide_approval_hold <hold_id> <decision> [--note TEXT]`. Both functions are idempotent: creating a hold with the same `(kind, source, summary[:120])` fingerprint while one is still `pending` returns the existing record; deciding an already-terminal hold returns the prior outcome. `list_approval_holds [--state STATE]` returns holds with optional state filter; `get_pending_holds` is the `/map-resume` integration point — `has_pending: true` means the orchestrator must stop before proceeding blindly. The `approval_hold` stage in `artifact_manifest.json` records `status:"pending"` while any hold is unresolved, and `status:"all_decided"` once all are terminal. Artifacts store only summaries and redacted metadata; no secrets, env values, tokens, or full sensitive payloads are ever persisted.
