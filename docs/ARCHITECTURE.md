...
- **Workflow Run Snapshots** (`create_workflow_snapshot`, issue #415): a content-addressed capture of the prompt/config surface that drove one MAP run, written to `.map/<branch>/snapshots/<run-id>/` (`manifest.json` + `skill.md` + `resolved-config.json`) via an atomic temp-dir rename, and registered as a `workflow_snapshot` artifact-manifest stage. `content_hash` covers every captured input (workflow, provider, branch, MAP identity, skill body, config, config state, each extra source), so identity is complete rather than partial: a rerun under the same `run_id` with changed inputs is a reported collision, and a rerun with identical inputs reuses the snapshot only after `_verify_snapshot_files` re-hashes the stored payload — a manifest match alone would let a half-deleted snapshot pass as valid. Every hashed string is written with `write_bytes`, so the digest describes the bytes actually on disk regardless of platform newline translation. The manifest always carries a MAP identity (`map_version`, else `map_source_sha256` from the runner's own digest — enforced by an `anyOf` in `WORKFLOW_SNAPSHOT_SCHEMA`) and distinguishes `absent`/`unreadable`/`present` config states so two genuinely different states cannot hash alike. **Secret containment is a design constraint, not a nicety**, because the artifact is durable and reviewer-visible: `run_id`/`workflow_id`/`branch` are validated as single path segments (the rule `_research_path` already applies) so none can escape the tree or make the runner read outside the project; `extra_sources` are confined to the project root, keyed by relative path, refused on credential-shaped names both as spelled and as resolved (symlinks), and refused on high-confidence secret content via the shared `_scan_outbound_secrets`; `resolved-config.json` redacts secret-shaped values by key name AND by scanning the values themselves, keeping keys so the configuration surface stays reconstructable. The stage registration is shared by the create and reuse paths and is best-effort: the snapshot is already durable when it runs, so a manifest failure is reported as `manifest_error` on the result rather than raised past a published artifact.
