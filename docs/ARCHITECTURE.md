...
- **PRD/Requirements-Quality Review** (`/map-prd-review`, issue #413): a pre-planning gate that reviews an externally supplied product brief, PRD, or requirements note across 11 dimensions before `/map-plan` is called. `write_prd_review()` in `map_step_runner.py` validates and writes `.map/<branch>/prd-review.json` and `.map/<branch>/prd-review.md`. Supported verdicts: `ready_for_plan` (`proceed: true`), `needs_prd_revision` (PRD has fixable gaps; `proceed: false`), `needs_user_decision` (concrete product/design choices must be answered; requires non-empty `blocking_questions`; `proceed: false`), `route_to_wayfind` (input too vague for a PRD review; `proceed: false`). Findings are validated per item (required fields: `dimension`, `severity`, `description`; optional: `suggested_revision`; severity must be one of `{critical, major, minor, info}`; no extra keys). Blocking questions require `question` and `category` keys. The `prd_review` manifest stage is updated after every write. CLI: `map_step_runner.py write_prd_review <verdict> [--findings '…'] [--blocking-questions '…'] [--suggested-revisions '…'] [--summary "…"] [--prd-source "…"]`. Registered in `skill-rules.json` as `type=manual`, `skillClass=task`.
...
Earlier delta (2026-08-13): PRD/requirements-quality review gate (#413): `write_prd_review()`
  writes `.map/<branch>/prd-review.{json,md}` with one of four verdicts
  (`ready_for_plan`, `needs_prd_revision`, `needs_user_decision`, `route_to_wayfind`).
  `needs_user_decision` requires non-empty `blocking_questions`. Findings validated
  per-item against `PRD_REVIEW_FINDING_SEVERITIES`; no extra keys allowed. New
  `prd_review` manifest stage. New `/map-prd-review` skill registered in
  `skill-rules.json`. 13 tests added; 4400 total tests pass.
