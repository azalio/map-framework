...
- **Context Budgeting & Compression**: MAP exposes compression policies and thresholds and treats budget control as a first-class workflow concern. Generated Actor `<map_context>` blocks are also capped by a deterministic estimated-token budget before prompt injection so long plans cannot silently crowd out current-subtask and dependency context.
