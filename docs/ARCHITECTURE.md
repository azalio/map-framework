...
3. **Provider Portability**: Keep workflow intent stable while allowing provider-specific orchestration surfaces (`.claude/commands/` vs `.codex/skills/`).
...
- `.claude/commands/`: Workflow command prompts (`map-*.md`) that define MAP sequences for Claude Code
- `.claude/skills/`: Skill definitions for flows maintained skill-first (notably `/map-learn`)
...
- **Generated provider surfaces**: `.claude/commands/`, `.claude/skills/`, and `.codex/` are the operational “runtime spec” consumed by providers.
