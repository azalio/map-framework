...
|---|---|
| How participants interact over time | Mermaid `sequenceDiagram` |
| Dependencies and relationships | Mermaid `graph` |
| Conditions and branches | Mermaid `flowchart` |
| Flow through layers or stages | Mermaid `flowchart LR` with `subgraph` |
| Lifecycle and status transitions | Mermaid `stateDiagram-v2` |
| Combinations of settings | table |
| Before / after | comparison table |

If the result is achieved through an exchange of requests and data between participants, use a `sequenceDiagram`. Show the initiator, the recipients, the data passed, the responses, and the final result. Show essential alternatives and errors as separate branches.
