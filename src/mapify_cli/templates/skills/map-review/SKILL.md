...
Role reviewers (`user_experience`, `maintainer`) — isolated context (diff +
bundle only), one JSON envelope each:
...
- `findings[]`: severity, category, file_path, line_range, symbol, evidence, and
  ...
  "none"). A finding missing any part is tombstoned as `contract_incomplete`:
  it never gates the change, and it is named in the ledger's `not_verified`.
