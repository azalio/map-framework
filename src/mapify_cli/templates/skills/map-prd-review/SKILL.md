...
| Verdict | Condition |
|---------|-----------|
| `ready_for_plan` | Score is at least 8.0, with no critical/major findings and no unresolved blocking decision. |
| `needs_user_decision` | A human must choose among materially different product, policy, design, or risk options. Include a blocking question. |
| `needs_prd_revision` | The direction is reviewable, but fixable gaps or score below 8.0 make it not ready. |
| `route_to_wayfind` | The input is too diffuse to identify a coherent feature and review it as a PRD. |
...
7. Persist the review and report the score, verdict, and artifact paths.
