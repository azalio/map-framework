...
- **Contract-sized subtasks** - `/map-plan` and `/map-efficient` require per-subtask `expected_diff_size`, `concern_type`, `one_logical_step`, and `coverage_map` metadata, then validate `blueprint.json` before implementation so oversized, mixed-concern, or untraceable acceptance-criteria plans fail early instead of surprising reviewers later.
