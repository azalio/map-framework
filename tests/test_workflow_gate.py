...
class TestWorkflowGate:
    ...
    def test_blocks_edit_without_actor(self, tmp_path: Path) -> None:
        ...
        response = json.loads(stderr.strip())
        assert response["decision"] == "block"
        assert "actor" in response["reason"].lower()
        assert "monitor" in response["reason"].lower()
    ...
    def test_blocks_when_no_current_subtask(self, tmp_path: Path) -> None:
        ...
        response = json.loads(stderr.strip())
        assert response["decision"] == "block"
        assert "current_subtask" in response["reason"].lower()
    ...
    def test_different_subtask_steps_independent(self, tmp_path: Path) -> None:
        ...
        response = json.loads(stderr.strip())
        assert response["decision"] == "block"
        assert "ST-002" in response["reason"] or "actor" in response["reason"].lower()
