...
class TestIntegrationWithRecitationManager:
    """Test integration with RecitationManager"""

    def test_logger_passed_to_recitation_manager(self, logger_enabled):
        """RecitationManager accepts logger instance"""
        from mapify_cli.recitation_manager import RecitationManager

        logger_enabled.start_session(task_id="integration_test")
        manager = RecitationManager(logger_enabled.project_root, logger=logger_enabled)

        assert manager.logger is logger_enabled

    def test_recitation_manager_logs_events(self, logger_enabled, temp_project):
        """RecitationManager logs events when logger is provided"""
        from mapify_cli.recitation_manager import RecitationManager
        ...
        # Should have: session_start + recitation_plan_created
        assert len(lines) >= 2

        plan_created_entry = json.loads(lines[1])
        assert plan_created_entry["event"] == "recitation_plan_created"
        assert plan_created_entry["task_id"] == "integration_test"
