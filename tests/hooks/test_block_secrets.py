...
HOOK_PATH = Path(__file__).parent.parent.parent / ".claude" / "hooks" / "block-secrets.py"
...
class TestPublicKeysAllowed:
    ...
    def test_public_keys_allowed(self, filename):
        exit_code, _, stderr = run_hook("Read", filename)
        assert exit_code == 0, f"Public key {filename} should be ALLOWED, got exit {exit_code}. stderr: {stderr}"
...
class TestGenericKeyFilesAllowed:
    ...
    def test_generic_key_files_allowed(self, filename):
        exit_code, _, stderr = run_hook("Read", filename)
        assert exit_code == 0, f"Generic key file {filename} should be ALLOWED, got exit {exit_code}. stderr: {stderr}"
...
class TestPathTraversal:
    ...
    def test_sensitive_path_component_blocked(self, path):
        ...
        assert exit_code == 2, f"Path with sensitive component '{path}' should be blocked. stderr: {stderr}"
