...
class TestContradictionDetector:
    ...
    def sample_data(self, db_conn):
        ...
        entities_data = [
            ...
            ('ent-generic-exception', 'ANTIPATTERN', 'generic-exception', 0.9, now, now),
            ('ent-specific-exceptions', 'PATTERN', 'specific-exceptions', 0.9, now, now),
            ('ent-silent-failure', 'ANTIPATTERN', 'silent-failure', 0.85, now, now),
            ('ent-explicit-error-handling', 'PATTERN', 'explicit-error-handling', 0.85, now, now),

            # Medium confidence entities
            ('ent-magic-numbers', 'ANTIPATTERN', 'magic-numbers', 0.7, yesterday, yesterday),
            ('ent-named-constants', 'PATTERN', 'named-constants', 0.75, now, now),

            # Low confidence entities
            ('ent-premature-optimization', 'ANTIPATTERN', 'premature-optimization', 0.6, yesterday, yesterday),
        ...
        relationships_data = [
            ...
            ('rel-004', 'ent-premature-optimization', 'ent-readable-code', 0.65,
        ...
        for rel_id, source_id, target_id, confidence, metadata in relationships_data:
            db_conn.execute("""
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
            """, (rel_id, source_id, target_id, 'CONTRADICTS', 'bullet-001',
    ...
    def test_detect_contradictions_confidence_filter(self, detector, db_conn, sample_data):
        """Test confidence threshold filtering."""
        # High confidence threshold: should exclude low confidence contradictions
        high_conf_contradictions = detector.detect_contradictions(db_conn, min_confidence=0.8)
    ...
    def test_find_entity_contradictions_bidirectional(self, detector, db_conn, sample_data):
    ...
    def test_find_entity_contradictions_no_conflicts(self, detector, db_conn, sample_data):
    ...
    def test_find_entity_contradictions_invalid_id(self, detector, db_conn):
        ...
        with pytest.raises(ValueError, match="Entity ID must start with 'ent-'"):
            detector.find_entity_contradictions(db_conn, 'invalid-id', min_confidence=0.7)
    ...
    def test_severity_low(self, detector, db_conn, sample_data):
        ...
        for contra in low_severity:
            ...
            both_low_conf_entities = (contra.entity_a.confidence < 0.6 and
                                      contra.entity_b.confidence < 0.6)
    ...
    def test_resolution_higher_confidence(self, detector, db_conn):
        ...
        db_conn.execute("""
            INSERT INTO entities (id, type, name, confidence, first_seen_at, last_seen_at,
                                  metadata, created_at, updated_at)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
        """, ('ent-low-conf', 'PATTERN', 'low-conf-pattern', 0.5, now, now, None, now, now))

        db_conn.execute("""
            INSERT INTO entities (id, type, name, confidence, first_seen_at, last_seen_at,
                                  metadata, created_at, updated_at)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
        """, ('ent-high-conf', 'PATTERN', 'high-conf-pattern', 0.9, now, now, None, now, now))
        ...
        db_conn.execute("""
            INSERT INTO relationships (id, source_entity_id, target_entity_id, type,
                                       ...
                                       created_at, updated_at)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
        """, ('rel-test', 'ent-low-conf', 'ent-high-conf', 'CONTRADICTS',
              'bullet-test', 0.8, None, now, now))

        db_conn.commit()

        conflicts = detector.find_entity_contradictions(db_conn, 'ent-low-conf', min_confidence=0.7)
    ...
    def test_report_group_by_severity(self, detector, db_conn, sample_data):
        ...
        report = detector.get_contradiction_report(db_conn, min_confidence=0.7, group_by='severity')

        assert report['total_count'] == 3  # Excludes low-confidence rel-004 (0.65 < 0.7)
    ...
    def test_report_group_by_entity_type(self, detector, db_conn, sample_data):
        ...
        report = detector.get_contradiction_report(db_conn, min_confidence=0.7, group_by='entity_type')
    ...
    def test_report_group_by_none(self, detector, db_conn, sample_data):
        ...
        report = detector.get_contradiction_report(db_conn, min_confidence=0.7, group_by='none')
    ...
    def test_report_invalid_group_by(self, detector, db_conn):
        ...
        with pytest.raises(ValueError, match="group_by must be"):
            detector.get_contradiction_report(db_conn, min_confidence=0.7, group_by='invalid')
    ...
    def test_performance_detect_contradictions(self, detector, db_conn, sample_data):
        ...
        assert elapsed_ms < 100, f"Performance target missed: {elapsed_ms:.2f}ms > 100ms"

    def test_performance_find_entity_contradictions(self, detector, db_conn, sample_data):
    ...
    def test_performance_check_new_pattern_conflicts(self, detector, db_conn, sample_data):
    ...
    def test_accuracy_on_test_corpus(self, detector, db_conn):
        ...
        test_cases = [
            ...
            ('ent-tc1-a', 'ANTIPATTERN', 'global-variables', 'ent-tc1-b', 'PATTERN', 'local-scope', 0.9, True),
            ('ent-tc2-a', 'ANTIPATTERN', 'hardcoded-values', 'ent-tc2-b', 'PATTERN', 'config-files', 0.85, True),
            ('ent-tc3-a', 'ANTIPATTERN', 'god-object', 'ent-tc3-b', 'PATTERN', 'single-responsibility', 0.9, True),
            ('ent-tc4-a', 'ANTIPATTERN', 'callback-hell', 'ent-tc4-b', 'PATTERN', 'async-await', 0.8, True),
            ('ent-tc5-a', 'ANTIPATTERN', 'spaghetti-code', 'ent-tc5-b', 'PATTERN', 'modular-design', 0.85, True),
            ('ent-tc6-a', 'ANTIPATTERN', 'copy-paste-code', 'ent-tc6-b', 'PATTERN', 'dry-principle', 0.9, True),
            ('ent-tc7-a', 'ANTIPATTERN', 'tight-coupling', 'ent-tc7-b', 'PATTERN', 'loose-coupling', 0.8, True),
            ('ent-tc8-a', 'ANTIPATTERN', 'no-error-handling', 'ent-tc8-b', 'PATTERN', 'try-catch-blocks', 0.85, True),
            ('ent-tc9-a', 'ANTIPATTERN', 'long-methods', 'ent-tc9-b', 'PATTERN', 'small-functions', 0.9, True),
            ('ent-tc10-a', 'ANTIPATTERN', 'deep-nesting', 'ent-tc10-b', 'PATTERN', 'early-returns', 0.8, True),
            ('ent-tc11-a', 'ANTIPATTERN', 'mutable-state', 'ent-tc11-b', 'PATTERN', 'immutability', 0.85, True),
            ('ent-tc12-a', 'ANTIPATTERN', 'synchronous-io', 'ent-tc12-b', 'PATTERN', 'async-io', 0.9, True),
            ('ent-tc13-a', 'ANTIPATTERN', 'manual-memory', 'ent-tc13-b', 'PATTERN', 'garbage-collection', 0.8, True),
            ('ent-tc14-a', 'ANTIPATTERN', 'stringly-typed', 'ent-tc14-b', 'PATTERN', 'strong-typing', 0.85, True),
            ('ent-tc15-a', 'ANTIPATTERN', 'no-tests', 'ent-tc15-b', 'PATTERN', 'test-coverage', 0.9, True),

            # False contradictions (noise - low confidence, should be filtered by min_confidence=0.7)
            ('ent-fc1-a', 'TOOL', 'pytest', 'ent-fc1-b', 'TOOL', 'unittest', 0.5, False),  # Not contradiction, alternatives
            ('ent-fc2-a', 'TECHNOLOGY', 'Python', 'ent-fc2-b', 'TECHNOLOGY', 'JavaScript', 0.4, False),
            ('ent-fc3-a', 'PATTERN', 'rest-api', 'ent-fc3-b', 'PATTERN', 'graphql-api', 0.6, False),
            ('ent-fc4-a', 'TOOL', 'docker', 'ent-fc4-b', 'TOOL', 'podman', 0.5, False),
            ('ent-fc5-a', 'WORKFLOW', 'agile', 'ent-fc5-b', 'WORKFLOW', 'waterfall', 0.6, False),
        ]

        # Insert entities and relationships
        for (ent_a_id, ent_a_type, ent_a_name,
             ent_b_id, ent_b_type, ent_b_name,
             rel_confidence, is_true_contradiction) in test_cases:
            ...
            db_conn.execute("""
                INSERT INTO relationships (id, source_entity_id, target_entity_id, type,
                                           ...
                                           created_at, updated_at)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
            """, (rel_id, ent_a_id, ent_b_id, 'CONTRADICTS', 'bullet-corpus',
                  rel_confidence, None, now, now))
        ...
        true_contradictions_count = sum(1 for tc in test_cases if tc[7])  # is_true_contradiction
        ...
        true_positive = sum(
            1 for (ent_a_id, _, _, ent_b_id, _, _, _, is_true) in test_cases
    ...
    def test_convenience_find_entity_contradictions(self, db_conn, sample_data):
        ...
        conflicts = find_entity_contradictions(db_conn, 'ent-generic-exception', min_confidence=0.7)
    ...
    def test_convenience_check_new_pattern_conflicts(self, db_conn, sample_data):
        ...
        conflicts = check_new_pattern_conflicts(db_conn, new_pattern, entities, min_confidence=0.7)
    ...
    def test_convenience_get_contradiction_report(self, db_conn, sample_data):
        ...
        report = get_contradiction_report(db_conn, min_confidence=0.7, group_by='severity')
    ...
    def test_contradiction_dataclass_validation(self):
        ...
        entity_a = Entity(id='ent-a', type=EntityType.PATTERN, name='pattern-a',
                          confidence=0.8, first_seen_at=now, last_seen_at=now)
        entity_b = Entity(id='ent-b', type=EntityType.PATTERN, name='pattern-b',
                          confidence=0.8, first_seen_at=now, last_seen_at=now)
        relationship = Relationship(id='rel-1', source_entity_id='ent-a',
                                    target_entity_id='ent-b', type=RelationshipType.CONTRADICTS,
                                    created_from_bullet_id='bullet-1', confidence=0.8)

        # Test valid severity values
        for severity in ['high', 'medium', 'low']:
            contra = Contradiction(
                id='contra-test', entity_a=entity_a, entity_b=entity_b,
                relationship=relationship, severity=severity,
                description='Test', resolution_suggestion='Test'
            )
        ...
        with pytest.raises(ValueError, match="Severity must be"):
            Contradiction(
                id='contra-test', entity_a=entity_a, entity_b=entity_b,
                relationship=relationship, severity='invalid',
                description='Test', resolution_suggestion='Test'
            )

        # Test invalid ID format
        with pytest.raises(ValueError, match="Contradiction ID must start with 'contra-'"):
            Contradiction(
                id='invalid-id', entity_a=entity_a, entity_b=entity_b,
                relationship=relationship, severity='high',
                description='Test', resolution_suggestion='Test'
            )
    ...
    def test_empty_database(self, detector, db_conn):
        ...
        conflicts = detector.find_entity_contradictions(db_conn, 'ent-nonexistent', min_confidence=0.7)
        assert conflicts == []
