...
from mapify_cli.playbook_query import PlaybookQuery, PlaybookResult, SearchMode
...
class TestCipherOnlyMode:
    ...
    def test_cipher_only_returns_cipher_results(
        self, manager_with_bullets, mock_cipher_callback
    ):
        ...
        params = PlaybookQuery(
            query="JWT authentication", search_mode=SearchMode.CIPHER_ONLY, limit=5
        )
    ...
    def test_cipher_only_metadata(self, manager_with_bullets, mock_cipher_callback):
        ...
        params = PlaybookQuery(query="JWT", search_mode=SearchMode.CIPHER_ONLY, limit=5)
...
class TestHybridMode:
    ...
    def test_hybrid_returns_both_sources(
        self, manager_with_bullets, mock_cipher_callback
    ):
        ...
        assert (
            len(sources) >= 1
        )  # At least one source (may not have both if deduplication removes all cipher)
    ...
    def test_hybrid_deduplicates_similar_results(
        self, manager_with_bullets, mock_cipher_callback
    ):
        """Test HYBRID mode deduplicates similar results (>85% threshold)"""
        manager_with_bullets.set_cipher_callback(mock_cipher_callback)

        params = PlaybookQuery(query="cookies", search_mode=SearchMode.HYBRID, limit=10)
    ...
    def test_hybrid_prefers_playbook_for_duplicates(
        self, manager_with_bullets, mock_cipher_callback
    ):
        ...
        params = PlaybookQuery(
            query="httpOnly cookies", search_mode=SearchMode.HYBRID, limit=10
        )
    ...
    def test_hybrid_metadata_timing(self, manager_with_bullets, mock_cipher_callback):
        ...
        params = PlaybookQuery(query="JWT", search_mode=SearchMode.HYBRID, limit=5)
    ...
    def test_merge_results_basic(self, manager_with_bullets):
        ...
        playbook_results = [
            PlaybookResult(
                id="sec-0001",
                section="SECURITY_PATTERNS",
                content="Unique playbook result",
                code_example=None,
                helpful_count=5,
                harmful_count=0,
                quality_score=5,
...
class TestMetadataTracking:
    ...
    def test_metadata_deduplication_count(
        self, manager_with_bullets, mock_cipher_callback
    ):
...
class TestBackwardCompatibility:
    ...
    def test_playbook_only_mode_unchanged(self, manager_with_bullets):
        ...
        params = PlaybookQuery(
            query="JWT", search_mode=SearchMode.PLAYBOOK_ONLY, limit=5
        )
    ...
    def test_get_relevant_bullets_still_works(self, manager_with_bullets):
        ...
        results = manager_with_bullets.get_relevant_bullets(query="JWT", limit=5)
