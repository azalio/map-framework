...
class TestCLIBehaviorWithNoPlan:
    ...
    def test_cli_update_no_plan_crashes(self, temp_project):
        ...
        assert 'NoneType' in response['message'] or 'No active plan' in response['message']
...
class TestForceFlagBehavior:
    ...
    def test_force_flag_works_when_no_plan_exists(self, manager, sample_subtasks):
        ...
        plan = manager.create_plan('feat_test', 'Test Goal', sample_subtasks, force=True)
...
class TestExpectedWorkflow:
    ...
    def test_abandoned_workflow_incomplete_then_new_plan(self, manager, sample_subtasks):
