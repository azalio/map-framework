...
class TemplateOptimizer:
        ...
        for i, (title1, content1) in enumerate(sections):
            for title2, content2 in sections[i+1:]:
                if len(content1) > 200 and len(content2) > 200:
                    ...
                    if similarity > 0.7:
                        similar.append((title1, title2, similarity, len(content1) + len(content2)))
    ...
    def suggest_consolidations(self, file_path: Path):
        ...
        if duplicates:
            suggestions.append(f"Found {len(duplicates)} redundant text blocks (5+ lines repeated)")

        # 2. Check example lengths
        code_blocks = self.analyze_example_lengths(content)
        long_blocks = [b for b in code_blocks if b[2] > 80]
        if long_blocks:
            suggestions.append(f"Found {len(long_blocks)} code examples >80 lines (could be shortened)")

        # 3. Check for similar sections
        similar_sections = self.find_redundant_sections(content)
        if similar_sections:
            suggestions.append(f"Found {len(similar_sections)} similar sections (>70% match)")

        # 4. Check critical tags
        critical_count, critical_chars = self.count_critical_tags(content)
        if critical_count > 3:
            suggestions.append(f"Found {critical_count} <critical> blocks ({critical_chars} chars total, could consolidate)")
    ...
    def estimate_savings(self, file_path: Path):
        ...
        if critical_count > 3:
            savings += (critical_count - 2) * 10  # Save ~10 lines per extra critical block
    ...
    def analyze_all(self):
        ...
        template_files = sorted([f for f in self.template_dir.glob('*.md')
        ...
        for file_path in template_files:
            ...
            if suggestions:
                for suggestion in suggestions:
                    print(f"   • {suggestion}")
                print(f"   💰 Potential savings: ~{savings} lines ({savings/lines*100:.1f}%)")
            else:
        ...
        print(f"Potential savings:    ~{total_savings} lines ({total_savings/total_lines*100:.1f}%)")
        print(f"Target (10-15%):      {int(total_lines * 0.10)}-{int(total_lines * 0.15)} lines")
        print(f"Status:               {'✓ Target achievable' if total_savings >= total_lines * 0.10 else '⚠ Below target'}")
        print()
...
def main():
    ...
    parser.add_argument('--analyze', action='store_true', help='Analyze templates and suggest optimizations')
