...
class TemplateLinter:
    ...
    def lint_yaml_frontmatter(self, file_path: Path, content: str):
        ...
        # Required fields
        required_fields = ['name', 'description', 'tools', 'model', 'version', 'last_updated', 'changelog']
        ...
        if 'version' in frontmatter:
            ...
            if not re.match(r'^\d+\.\d+\.\d+$', str(version)):
                self.error(file_path.name, end_line, f"Invalid version format: {version} (expected semver: X.Y.Z)")

        # Validate last_updated format (YYYY-MM-DD)
        if 'last_updated' in frontmatter:
            date = str(frontmatter['last_updated'])
            if not re.match(r'^\d{4}-\d{2}-\d{2}$', date):
                self.error(file_path.name, end_line, f"Invalid date format: {date} (expected YYYY-MM-DD)")
    ...
    def lint_required_sections(self, file_path: Path, content: str):
        ...
        if missing:
            self.warning(file_path.name, 0, f"Missing recommended sections: {', '.join(missing)}")
        ...
        for i, line in enumerate(lines, 1):
            ...
            for match in matches:
                ...
                if '{{' in var_content or '}}' in var_content:
                    self.error(file_path.name, i, f"Malformed template variable: {{{{{var_content}}}}}")

                # Check for common variable names (for consistency)
                common_vars = ['project_name', 'language', 'framework', 'subtask_description',
                              'playbook_bullets', 'feedback', 'standards_url', 'branch', 'related_files']
                ...
                if is_closing:
                    if not tag_stack:
                        ...
                        if tag_name.lower() not in inline_tags:
                            self.warning(file_path.name, i, f"Closing tag </{tag_name}> without opening tag")
                        ...
                        if tag_in_stack:
                            ...
                            while tag_stack and tag_stack[-1][0] != tag_name:
                                ...
                                self.info(file_path.name, unclosed[1],
                                        f"Tag <{unclosed[0]}> was not explicitly closed before </{tag_name}>")
                            if tag_stack:
                                tag_stack.pop()
                        else:
                            self.warning(file_path.name, i,
    ...
    def lint_mcp_tool_descriptions(self, file_path: Path, content: str):
        ...
        for tool, keywords in mcp_tools.items():
            if tool in content:
                ...
                missing_keywords = [kw for kw in keywords if kw.lower() not in tool_section.lower()]

                if len(missing_keywords) > len(keywords) // 2:
                    self.info(file_path.name, 0,
        ...
        for i, line in enumerate(lines, 1):
            if line.strip().startswith('```'):
                if not in_code_block:
                    ...
                    if block_length < 5 and 'example' in content[max(0, code_block_start - 200):code_block_start].lower():
                        self.warning(file_path.name, code_block_start,
    ...
    def lint_section_naming_consistency(self):
        ...
        for file_path in self.template_dir.glob('*.md'):
            ...
            for header in markdown_headers:
                section_names[header.lower()].append((file_path.name, 'markdown', header))
        ...
        mcp_variants = [name for name in section_names if 'mcp' in name and 'integration' in name]
        if len(set(mcp_variants)) > 1:
            self.warning('*', 0, f"Inconsistent MCP section naming: {set(mcp_variants)}")
    ...
    def lint_all(self):
        ...
        template_files = [f for f in template_files if f.name not in ['CHANGELOG.md', 'README.md', 'MCP-PATTERNS.md']]
    ...
    def print_results(self):
        ...
        if var_stats:
            ...
            for var, count in sorted(var_stats.items(), key=lambda x: x[1], reverse=True)[:10]:
        ...
        if self.stats['errors'] > 0:
            print(f"{Colors.RED}✗ Linting failed with {self.stats['errors']} error(s){Colors.END}")
            return 1
        elif self.stats['warnings'] > 0:
            print(f"{Colors.YELLOW}⚠ Linting passed with {self.stats['warnings']} warning(s){Colors.END}")
            return 0
        else:
...
def main():
    ...
    parser.add_argument('--dir', type=str, default='.claude/agents', help='Template directory')
